...
const Review = ({ recipeId }) => {
    ...
    const getReviews = React.useCallback(async (recipe_id) => {
        try {
            // Get review information (reviews and average rating)
            const response = await Api.callApiGetReviews(recipe_id);
            const reviews = response.reviews || []; // Get only the reviews array
            setReviews(reviews);
            setAverageRating(response.average_rating); // Set the average rating
        } catch (error) {
